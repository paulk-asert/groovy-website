...
A reference implementation of the `switch` and `instanceof` pattern features has been
developed against Groovy master; learnings from that work are folded into this
document. Bracket-form assignment is deferred pending further review (see below).
____
...
component grammar is not expression grammar. One carve-out results:
a call label whose sole argument is `_` — in practice only
`case foo(_) \->` with `_` in scope as a variable — reparses as a
record pattern with a wildcard component. Call labels with any other
argument shapes (`case foo()`, `case foo(bar)`, `case lower('ABC')`)
keep their legacy `isCase` semantics; record patterns are only
recognised when every component is pattern-shaped.
...
no functionality. Since patterns require the arrow form, the
colon-form spellings `case []:` and `case [:]:` — previously
compiling, never-matching dead code — become compile-time errors
rather than silently keeping their useless legacy meaning.
...
* The static type checker additionally reports an *error* for a
  ...
  *warns* for an arm dominated by a preceding label (unreachable) and
  for a non-exhaustive pattern switch (a `default` branch, an
  unconditional pattern, or full sealed-hierarchy coverage each
  satisfy it). These are static-typing diagnostics only; dynamic
  Groovy remains permissive.
...
* `case foo(_) \->` with `_` in scope as a variable: previously a
  method-call label passing `_`, now a record pattern with a wildcard
  component (see _Record patterns_).
* Colon-form `case []:` and `case [:]:` (and colon-form pattern-shaped
  labels generally): previously compiling, now compile-time errors,
  since patterns require the arrow form. The legacy colon forms could
  never match.

No realistic program relies on any of these legacy meanings; they are
